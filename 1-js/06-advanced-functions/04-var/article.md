...
En el primer captítulo sobre [variables](info:variables), mencionamos tres maneras de declarar las variables: 

1. `let`
2. `const`
3. `var`
                
`let` y `const` se compartan por igual en el ambiente léxico.

Pero `var` is a very different beast, that originates from very old times. It's generally not used in modern scripts, but still lurks in the old ones.
